...
public class PatientHelperService extends IOIOService{
	...
	private final static int SD_OUTPUT = 12;
	...
	public void bufferCleanup()
	{
		...
		{
			if(checkConnection(this)){//should check if you are on wifi before attempting to send data
			        ...
			        public void run()
			        {
			        	for(int i = 0; i < tempSize; i++)
						{
							...
							if(gpsStrings.length>8&&tempData.gps.substring(0,tempData.gps.length()-5).contains("A")){
								...
								data.add(new BasicNameValuePair("latitude", gpsNS));
								data.add(new BasicNameValuePair("longitude",gpsEW));
								data.add(new BasicNameValuePair("velocity",gpsVelocity));
							}
							else{
								data.add(new BasicNameValuePair("gps", tempData.gps));
							}
							data.add(new BasicNameValuePair("date", tempData.date));
							data.add(new BasicNameValuePair("id", android_id));
							
			                PostToServer postMan=new PostToServer();
			                final String temp=postMan.postResults(data,"http://mscs-php.uwstout.edu/geoinet/EBPHPCode/environmentalParse.php");
						} 
							...
							try {
								while((line=br.readLine())!=null){
									if(!(line.substring(0,4).equals("Year"))){
										...
										data.add(new BasicNameValuePair("id",android_id));
										data.add(new BasicNameValuePair("date",line.substring(0,19)));
										data.add(new BasicNameValuePair("remaining",line.substring(20)));
										PostToServer postMan=new PostToServer();
						                final String temp=postMan.postResults(data,"http://mscs-php.uwstout.edu/geoinet/EBPHPCode/environmentalParse.php");
									}
								}
							...
							try {
								while((line=br.readLine())!=null){
									if(!(line.substring(0,4).equals("Year"))){
										...
										filedata.add(new BasicNameValuePair("date",line.substring(0,19)));
										filedata.add(new BasicNameValuePair("remaining",line.substring(18)));
										PostToServer filepostMan=new PostToServer();
						                final String filetemp=filepostMan.postResults(filedata,"http://mscs-php.uwstout.edu/geoinet/EBPHPCode/attackParse.php");
									}
								}
							...
							try {
								while((line=br.readLine())!=null){
									if(!(line.substring(0,4).equals("Year"))){
										...
										filedata.add(new BasicNameValuePair("id",android_id));
										filedata.add(new BasicNameValuePair("date",line.substring(0,19)));
										filedata.add(new BasicNameValuePair("remaining",line.substring(19)));
										PostToServer filepostMan=new PostToServer();
						                final String filetemp=filepostMan.postResults(filedata,"http://mscs-php.uwstout.edu/geoinet/EBPHPCode/medicationParse.php");
									}
								}
							...
							try {
								while((line=br.readLine())!=null){
									if(!(line.substring(0,4).equals("Year"))){
										...
										filedata.add(new BasicNameValuePair("id",android_id));
										filedata.add(new BasicNameValuePair("date",line.substring(0,19)));
										filedata.add(new BasicNameValuePair("remaining",line.substring(20)));
										PostToServer filepostMan=new PostToServer();
						                final String filetemp=filepostMan.postResults(filedata,"http://mscs-php.uwstout.edu/geoinet/EBPHPCode/peakFlowParse.php");
									}
								}
			...
			Log.d("DataDump", "Dumping buffer to textfile");
				for(int i = 0; i < tempSize; i++)
				{
					...
					try
					{
						String tempString=tempData.date+","+tempData.temperature+","+tempData.humidity+","+tempData.gps+"\r\n";
						File externalMem=Environment.getExternalStorageDirectory();
						File directory=new File (externalMem.getAbsolutePath()+"/PatientHelper");
						directory.mkdirs();
						File file=new File(directory,"environmentalData.txt");
						...
						if(file.exists()){
							...
							fw.write("Year-Month-Day,Hour:Minute:Second,Temperature,Humidity,Degrees,Direction,Degrees,Direction,Speed\r\n");//add the information as to what is between each comma.
							fw.write(tempString);
							fw.close();
						}
	...
	private class SensorData
	{
		...
		public SensorData(float t, float h, String g)
		{
			temperature = t;
			humidity = h;
			gps = g;
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			{	
				...
				humid = Math.round(humid *10) / 10.0f;
					...
					SensorData incData = new SensorData(temp, humid, gps);//change test back to gps with quotes
					buffer.add(incData);
